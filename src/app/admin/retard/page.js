...
export default function RetardListPage() {
    ...
    const fetchAgents = async (userIdd) => {
        try {
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            <button onClick={() => setShowModal(false)}
                                    className="px-4 py-2 bg-gray-400 text-white rounded-md">Annuler
                            </button>
                            <button onClick={handleAddDelay}
                                    className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                                {isSubmitting ? (
                                    <div
                                        className="animate-spin h-5 w-5 border-4 border-blue-600 border-t-transparent rounded-full"></div>
                                ) : 'Ajouter'}
                            </button>
                        </div>
                    </div>
