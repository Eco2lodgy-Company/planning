...
import {Calendar, Check, ChevronDown, Loader2, Plus, Search, X, XCircle} from 'lucide-react';
...
//   }, [router]);
    ...
    const fetchLeaves = async () => {
        const userId = localStorage.getItem("userId");
        try {
            const response = await fetch('/api/headside/permissions/' + userId);
                ...
                <div className="p-6">
                    ...
                    >
                        <div className="max-h-[800px] overflow-y-auto">
                          ...
                          <span
                              className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(permission.status)}`}>
                            {permission.status}
                          </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            {filteredPermissions.length === 0 && (
                                <div className="text-center py-12">
                                    ...
                                    <h3 className="mt-2 text-sm font-medium text-gray-900">Aucune permission
                                        trouvée</h3>
                                    <p className="mt-1 text-sm text-gray-500">Essayez d'ajuster votre recherche ou vos
                                        filtres.</p>
                                </div>
                            )}
                        </div>
                ...
                {isPopupOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Type de
                                        permission</label>
                                    <select
